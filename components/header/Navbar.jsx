...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-20 transition-colors duration-300 backdrop-blur-lg 
      ${pathname === "/" ? "bg-black/20" : "bg-slate-800"}`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6 lg:px-8">
        <Link href="/" aria-current="page" className="flex items-center gap-3">
          <Logo fillLotus="fill-slate-200" fillFlower="fill-green-600" />
      ...
      {mobileOpen && (
        <div className="mx-auto mt-2 w-[calc(100%-1rem)] max-w-7xl rounded-lg border border-white/15 bg-black/70 px-4 py-3 backdrop-blur md:hidden">
          <nav className="grid gap-3">
            ...
            <Link href="/contact" className="mt-2 inline-flex items-center justify-center gap-2 rounded-md border border-white/20 px-4 py-2 text-white/90 hover:bg-white/10 transition">
              <span className="text-sm font-medium uppercase tracking-wide">
                contact us
              </span>
              <MdArrowOutward className="text-white/90 h-4 w-4" />
